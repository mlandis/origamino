...
MbTransitionMatrix::MbTransitionMatrix(const std::vector<double> &rate, bool useEigen)
	...
	if ( rate.size() != (unsigned int)(nSt*(nSt-1)) )
		return;
...
MbTransitionMatrix::MbTransitionMatrix(const std::vector<double> &rate, const std::vector<double> &pi, bool useEigen)
	...
	int nSt = pi.size();
	if ( 2*rate.size() != (unsigned int)(nSt*(nSt-1)) )
...
MbTransitionMatrix::MbTransitionMatrix(const MbMatrix<double> &qMat, bool useEigen)
	...
	// verify qMat contents
#	if 0
	for (int i = 0; i < numStates; i++) {
		for (int j = 0; j < numStates; j++) {
			std::cout << "qMat[" << i << "," << j << "]" << std::fixed << std::setprecision(8) << (double)qMat[i][j] << std::endl;
		}
	}
#	endif
...
void MbTransitionMatrix::calcComplexCijk(void) {
	...
	for (int i=0; i<numStates; i++)
		ceigenvalue[i] = std::complex<double>(*p++, *q++);

	// calculate cc_ijk
	MbMatrix<std::complex<double> > cev = eigens->getComplexEigenvectors();
	MbMatrix<std::complex<double> > ciev = eigens->getComplexInverseEigenvectors();
	std::complex<double> *pc = cc_ijk;
	for (int i=0; i<numStates; i++)
		for (int j=0; j<numStates; j++)
			for (int k=0; k<numStates; k++)
			 	*(pc++) = cev[i][k] * ciev[k][j];
...
void MbTransitionMatrix::restoreQ(void) {
	...
	if (useEigens) {
		...
		if ( isComplex || isOldComplex ) {
			std::complex<double> *tempC = ceigenvalue;
...
void MbTransitionMatrix::tiProbsComplexEigens(const double v, MbMatrix<double> &P) {
	...
	const std::complex<double> *ptr = cc_ijk;
	for (int i=0; i<numStates; i++) 
		{
		for (int j=0; j<numStates; j++) 
			{
...
/*!
 * This function calculates the uniformized rate matrix for Q. Note that 
 * the matrix may not be instantiated when we arrive to this function.
 *
 * \brief Calculate uniformized rate matrix
 *
 * \return A reference to the uniformized rate matrix
 *
 */
MbMatrix<double> &MbTransitionMatrix::uniformize(double *mu) {

	if (hasUniformizedMatrix == false)
		{
		uniformizedQ = MbMatrix<double>(numStates, numStates);
		hasUniformizedMatrix = true;
		}
	...
	for (int i=0; i<numStates; i++)
		for (int j=0; j<numStates; j++)
			uniformizedQ[i][j] = Q[i][j] / (*mu);
	for (int i=0; i<numStates; i++)
		uniformizedQ[i][i] += 1.0;
	return uniformizedQ;
}
 ...
 */
int MbTransitionMatrix::updateQ(const std::vector<double> &rate) {
    
	if ( rate.size() != (unsigned int)(numStates*(numStates-1)) )
		return (1);
 ...
 */
int MbTransitionMatrix::updateQ(const std::vector<double> &rate, const std::vector<double> &pi) {
    
	if ( (2*rate.size() != (unsigned int)(numStates*(numStates-1))) || (pi.size() != (unsigned int)numStates) )
		return (1);
 ...
 * \return 0 for success, 1 for failure (wrong dimensions on rate or pi)
 */
int MbTransitionMatrix::updateQ(const MbMatrix<double> &qTemp) {
